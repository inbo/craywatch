...
# 2: som van vallen en individuen per dag en per locatie, soort en sessie
...
grouped_craywatch_data <- daily_data %>%
  ...
  mutate(
    by_protocol = if_else(
      (soort == "crayfish indet" & traps_used >= 12) | (soort != "crayfish indet"),
      TRUE,
      FALSE
    )
  ) %>%
...
write.csv(grouped_craywatch_data, "~/GitHub/craywatch/R/data/output/final_craywatch_data_all.csv", row.names = FALSE)
